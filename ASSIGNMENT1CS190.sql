...
SELECT cust_name FROM customer where cust_balance < cust_limit; 
...
SELECT MIN(cust_balance) AS MINIMUM_BALANCE FROM customer;
...
SELECT rep_id, SUM(cust_balance) FROM customer GROUP BY rep_id HAVING SUM(customer_balance)>12000 ORDER BY rep_id;
-- 21.	Display the ids of all products whose description is not known.
SELECT prod_id from product where prod_desc='None';